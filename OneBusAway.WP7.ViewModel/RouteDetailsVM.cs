...
namespace OneBusAway.ViewModel
{
    public class RouteDetailsVM : AViewModel
    {
        ...
        {
            _arrivalsForStop = new List<ArrivalAndDeparture>();
            if (UnfilteredArrivals != null)
            {
                if (routeFilter != null)
                {
                    var filteredArrivals = UnfilteredArrivals.Where(arrival => arrival.routeId == routeFilter.id);
                    _arrivalsForStop.AddRange(filteredArrivals);
                }
                else
                {
                    _arrivalsForStop.AddRange(UnfilteredArrivals);
                }
            }
        ...
        public async void RefreshArrivalsForStopAsync(Stop stop)
        {
            if (stop != null)
            {
                ...
                var arrivals = await BusServiceModel.ArrivalsForStopAsync(LocationTracker.CurrentLocation, stop);
